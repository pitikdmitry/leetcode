'''
Given an array points containing the coordinates of points on a 2D plane, sorted by the x-values,
where points[i] = [xi, yi] such that xi < xj for all 1 <= i < j <= points.length. You are also given an integer k.

Find the maximum value of the equation yi + yj + |xi - xj| where |xi - xj| <= k and 1 <= i < j <= points.length.
It is guaranteed that there exists at least one pair of points that satisfy the constraint |xi - xj| <= k.


Example 1:

Input: points = [[1,3],[2,0],[5,10],[6,-10]], k = 1
Output: 4
Explanation: The first two points satisfy the condition |xi - xj| <= 1 and if we calculate the equation we get 3 + 0 + |1 - 2| = 4. Third and fourth points also satisfy the condition and give a value of 10 + -10 + |5 - 6| = 1.
No other pairs satisfy the condition, so we return the max of 4 and 1.
'''
from collections import deque
from typing import List


class Solution:
    def findMaxValueOfEquation(self, points: List[List[int]], k: int) -> int:
        decreasing_queue = deque()
        j = 0
        max_res = 0
        for i in range(len(points)):
            x, y = points[i]

            while j < i and x - points[j][0] > k:
                j += 1

            #   remove old elements from queue
            while len(decreasing_queue) > 0 and decreasing_queue[0] < j:
                decreasing_queue.popleft()

            last_popped_value = None
            #   maintain decreasing queue, but
            while len(decreasing_queue) > 0 and decreasing_queue[-1][1] < y:
                last_popped_value = decreasing_queue.pop()

            decreasing_queue.append(i)

            if len(decreasing_queue) == 1 and last_popped_value is not None:
